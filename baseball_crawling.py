...
def job():
    ...
    # 날짜에 맞는 URL 가져오기
    url = f"https://statiz.sporki.com/schedule/?year={yesterday_year}&month={yesterday_month}"
    response = requests.get(url)
    soup = BeautifulSoup(response.text, "html.parser")

    print(f"==== {yesterday_month}월 {yesterday_day}일 경기 정보 시작 ====\n")
    ...
    for day_element in day_elements:
        ...
        if game_day == yesterday_day:  # 하루 전 날짜와 일치하는 경우
            ...
            if parent_td:
                ...
                for game in games:
                    ...
                    FROM GameResult
                    WHERE home_team = %s AND away_team = %s AND DATE(game_date) = %s AND stadium = %s
                    """, (teams[1].text, teams[0].text, game_date, stadium))
                    
                    result = cursor.fetchone()
                                        
                    if result:
                        game_result_id = result[0]
                    
                        # 점수 업데이트
                        update_query = """
                        UPDATE GameResult 
                        SET score_home_team = %s, score_away_team = %s
                        WHERE game_result_id = %s
                        """
                        cursor.execute(update_query, (scores[1].text, scores[0].text, game_result_id))
